...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/scoring/whale-score.ts"
      provides: "Whale score calculation logic"
      exports: ["calculateWhaleScore"]
  ...
  key_links:
    ...
    - from: "src/app/api/cron/poll-whale-trades/route.ts"
      to: "src/lib/scoring/whale-score.ts"
      via: "calculateWhaleScore import"
      pattern: "import.*calculateWhaleScore"
---
...
Output: birdeye-whale-queries.ts with CRUD operations, whale-score.ts with scoring logic, poll-whale-trades cron route.
</objective>
...
<task type="auto">
  ...
  <files>src/lib/scoring/whale-score.ts, src/app/api/cron/poll-whale-trades/route.ts</files>
  <action>
**PREREQUISITE CHECK:** Before implementing, verify Plan 02-01 exports exist:

```bash
# Verify getWhaleTrades and aggregateWhaleTrades are exported from birdeye.ts
grep -E "export.*(function|async).*getWhaleTrades|export.*(function).*aggregateWhaleTrades" src/lib/apis/birdeye.ts

# Verify BirdeyeWhaleTrade type exists in types
grep "interface BirdeyeWhaleTrade" src/lib/types/index.ts
```

If these exports don't exist, STOP and report that Plan 02-01 must complete first.

**Step 1:** Create src/lib/scoring/whale-score.ts (extracted for testability):
...
**Step 2:** Create src/app/api/cron/poll-whale-trades/route.ts following the pattern from poll-birdeye/route.ts:
...
- calculateWhaleScore extracted to src/lib/scoring/whale-score.ts for testability
  </action>
  <verify>
Run `npm run build` to verify TypeScript compiles.
Test locally with `curl http://localhost:3000/api/cron/poll-whale-trades` (dev mode).
Verify calculateWhaleScore is importable: `grep "calculateWhaleScore" src/app/api/cron/poll-whale-trades/route.ts`
  </verify>
  <done>
poll-whale-trades cron job exists, fetches whale trades from Birdeye, calculates scores using extracted whale-score.ts, stores in database.
  </done>
</task>
...
5. Score logic extracted: calculateWhaleScore exists in src/lib/scoring/whale-score.ts
</verification>

<success_criteria>
- birdeye-whale-queries.ts exports upsert and query functions
- whale-score.ts exports calculateWhaleScore with documented scoring factors
- poll-whale-trades cron job polls top 20 tokens per chain for whale activity
- Whale score calculated using extracted calculateWhaleScore function
